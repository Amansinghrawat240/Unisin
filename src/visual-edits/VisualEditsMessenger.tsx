...
export const CHANNEL = "UNISIN_HOVER_v1" as const;
...
const getCurrentStyles = (element: HTMLElement): Record<string, string> => {
...
export default function HoverReceiver() {
  ...
  const [isVisualEditMode, setIsVisualEditMode] = useState(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("unisin_visual_edit_mode");
      return stored === "true";
    }
    return false;
  });
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("unisin_visual_edit_mode", String(isVisualEditMode));
    }
  }, [isVisualEditMode]);
  ...
  useEffect(() => {
    if (isVisualEditMode) {
      ...
      setTimeout(() => {
        if (typeof window !== "undefined") {
          const focusedData = localStorage.getItem("unisin_focused_element");
          if (focusedData) {
            try {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-unisin-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-unisin-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    const unisinId = element.getAttribute("data-unisin-id");
    if (!unisinId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseUnisinId(unisinId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    const existingStyles = appliedStylesRef.current.get(unisinId) || {};
    appliedStylesRef.current.set(unisinId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(unisinId);
  ...
  const flushImageSrcChange = () => {
    ...
    const unisinId = imgElement.getAttribute("data-unisin-id");
    if (!unisinId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "UNISIN_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        const allMatchingElements = document.querySelectorAll(
          `[data-unisin-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "UNISIN_IMAGE_UPDATE") {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        if (parent) {
          ...
          const parentInnerHeight = parentRect.width - parentPaddingTop - parentPaddingBottom;
  ...
  useEffect(() => {
    function onPointerMove(e: PointerEvent) {
      ...
      const hit = document
        ...
        ?.closest<HTMLElement>("[data-unisin-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-unisin-id");
        ...
        const allMatchingElements = document.querySelectorAll(
          `[data-unisin-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;

        const boxes: Box[] = [];
        allMatchingElements.forEach((element) => {
          const elementId = element.getAttribute("data-unisin-id");
    ...
    function onMouseDownCapture(e: MouseEvent) {
      ...
      const hit = (e.target as HTMLElement)?.closest<HTMLElement>("[data-unisin-id]");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-unisin-id]");
      ...
      if (hit) {
        ...
        if (hitId && typeof window !== "undefined") {
          ...
          localStorage.setItem("unisin_focused_element", JSON.stringify(focusedElementData));
        }
        ...
        if (focusedElementRef.current) {
          ...
          if (typeof window !== "undefined") {
            localStorage.removeItem("unisin_focused_element");
          }
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "VISUAL_EDIT_MODE" && "active" in e.data) {
        ...
        if (!newMode && typeof window !== "undefined") {
          localStorage.removeItem("unisin_visual_edit_mode");
          localStorage.removeItem("unisin_focused_element");
        }
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        const allMatchingElements = document.querySelectorAll(
          `[data-unisin-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;

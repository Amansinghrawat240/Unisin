...
export async function POST(
  ...
  try {
    ...
    const currentTrack = trackSubmission[0];

    // Business logic: Update state and processing log
    const currentTimestamp = new Date();
    const newProcessingLog = currentTrack.processingLog 
      ? `${currentTrack.processingLog}\n${currentTimestamp.toISOString()}: submitted for review`
      : `${currentTimestamp.toISOString()}: submitted for review`;
    ...
    const updatedTrack = await db.update(trackSubmissions)
      .set({
        state: 'needs_review',
        processingLog: newProcessingLog,
    ...
    console.error('POST submit-for-review error:', error);
    return NextResponse.json({ 
